...
Its 10 questions in three parts cover what a wrong heritability does to the
score, how far a posterior mean liability is from a disease risk, and
estimating heritability from the register itself with a tetrachoric
correlation. Needs Python 3.9+ with NumPy, SciPy and ltpred installed from
source (not on PyPI; see the notebook's Setup section) — no data files, no R,
and Windows, macOS and Linux behave the same. Part 0 rebuilds the
[ltpred tutorial](https://bvilhjal.github.io/ltpred/tutorial/) register from
fixed seeds, so no prior session is needed. Parts A and B take one hour; Part
C is homework. The handouts were executed on 21 September with ltpred 0.7.0
on a simulated register whose genetic liability is known.
