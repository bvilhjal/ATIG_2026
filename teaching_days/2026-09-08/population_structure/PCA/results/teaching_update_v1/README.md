...
The `data/` folder contains the cached input files; `download_1000G()` reuses
these and downloads them if absent. PLINK 2 is downloaded for the detected
architecture if it is absent. When transferring the folder to another platform,
remove the cached PLINK executable so the appropriate build is downloaded.
...
To inspect the fitted results without recomputing PCA, use
`readRDS("results/pca_results.rds")` in R.

## Teaching update

The 2026 handouts add population-level interpretation of PCs, sensitivity to
the outlier cutoff, and a comparison that reveals changes in the ordering of
later PCs. They distinguish singular vectors from scores, explain how reference
scaling is reused in projection, and discuss why a variance target does not
determine the number of GWAS covariates. All 14 questions are retained; the
worked answers contain eight numbered figures and two tables.

The original full computation passed 13 checks. The teaching refresh reused
those fits, recalculated the variance and new summaries, and passed 21 checks.
To repeat that refresh from this folder using the supplied saved results:
...
`results/full_run_sources/`, and refresh outputs are in `results/teaching_update/`.
